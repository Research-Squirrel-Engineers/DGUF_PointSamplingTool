...
We tested the software for its performance in processing a large set of points overlaying a vector layer with overlapping polygons. For this purpose we selected more or less randomly a rectangular area of 240 x 160 km in northern Germany and created two layers with point features and one with polygon features. The first point layer (A) comprises 360 features spaced 10.000 m apart. These points were buffered with a radius of 22.000 m. The second point layer (B) comprises 16.905 features with a slight offset from point layer A. We performed point sampling of layers A and B on the layer with the overlapping buffers with the effects described above. 